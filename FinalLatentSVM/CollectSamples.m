function [svm_mod posFeatures negSamples negFeatures numBlocks] = CollectSamples(im0, template, templateOrigin, N, blockSize, TempSize)
...
[posFeatures numBlocks] = GenerateFeaturesInit(resizedTemp, blockSize);
%posFeatures = [posFeatures; GenerateFeatures(reverseTemp, blockSize)];
    ...
    for i=1:numHorizontalChange
        ...
        if (bottom < templateOrigin(2) && top < templateOrigin(2)) || (top > templateOrigin(2)+sizeofTemplate(1) && bottom > templateOrigin(2)+sizeofTemplate(1))
            if left > 0 && right < cols && top > 0 && bottom < rows
                ...
                cnt = cnt+1;
                resizedNegTemp = imresize(temp, TempSize);
                negSamples(cnt).image = resizedNegTemp;
                disp(cnt);
                negFeatures(cnt, :) = GenerateFeatures(resizedNegTemp, blockSize);
            end
