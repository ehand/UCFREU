...
function pastBlocks = collectPastBlocks(pastBlocks, map, template, blocksWide, blocksHigh, blockSize, shift, threshold)

newBlocks = templateToBlockSet(template, blockSize, shift, blocksHigh, blocksWide);
count=0;
for j=1:size(map, 1)
    for i=1:size(map,2)
        count=count+1;
